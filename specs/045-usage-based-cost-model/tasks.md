...
- [ ] T057 Produce the restatement announcement covering **both** restatements: per-user monthly totals before/after for every restated month (phase 4), and expected-spend before/after per period (phase 5), with the reason for each. A deliverable, not a follow-up
- [ ] T058 [P] Update `CLAUDE.md` Recent Changes and the `docs/anthropic-cost-accuracy.md` pointer once the feature lands
...
- Phase 8 is independent of phases 2–7 and can ship any time after phase 1. **T054/T055 (boost-\* deprecation) are the earliest available visible win.**
