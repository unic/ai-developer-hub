...
**Scale/Scope**: ~23 workspaces, 40 API-key assignments, 4 active API users, ~12k line-item rows/month. 4 new source modules, ~18 files touched
...
| 7   | **`group_by[]=description` pushes the sync past `maxDuration`**                                                                                                                        | Chunked upserts as today; backfill already loops month-by-month; measure on the first backfill before enabling in cron                                                                     |
