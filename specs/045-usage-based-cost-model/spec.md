...
- **OQ-3**: ~~What revocation date should the pooled assignments carry?~~ **Resolved 2026-09-18: `2026-07-01`.** Consistent with org-wide Claude API spend collapsing from $3,016.86 (June 2026) to $194.89 (July 2026). Budget periods ending before that date keep counting these assignments; periods from July onward do not.
- **OQ-4**: ~~What happens to assignment 262 (`boost-advanced`, `Automations` workspace)?~~ **Resolved 2026-09-18: revoke it too**, on the same date. It keeps its workspace's spend visible — that spend is attributed to the `Automations` workspace, not to the assignment, and `Automations` is a project workspace whose attribution is out of scope for this feature (it stays `unattributed`). FR-029 still holds for any _future_ boost-tier assignment found outside a pooled workspace.
