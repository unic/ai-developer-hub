...
- [x] T001 [P] Create server action `previewProfileApi` in `src/actions/profile-api-preview.ts` — accepts `{ email: string; month?: string }`, constructs base URL (NEXTAUTH_URL || VERCEL_URL || localhost:3000), calls `GET /api/profile?email=...&month=...` with Bearer `PROFILE_API_SECRET` header, measures response time via `performance.now()`, returns `{ status, statusText, responseTimeMs, body }` typed as `ApiPreviewResponse`. Include check: if `PROFILE_API_SECRET` is not set, return early with `{ success: false, error: "API not configured" }`. Validate inputs with Zod before sending.
- [x] T002 [P] Ensure `src/lib/validators.ts` exports reusable `apiPreviewEmailSchema` (z.string().email()) and `apiPreviewMonthSchema` (z.string().regex for YYYY-MM) — if these already exist as inline schemas, extract and export them so the server action and client form can share them.
