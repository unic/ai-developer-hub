...
export async function run(
  ...
  return withSyncLock(
    ...
    async () => {
      ...
      // Backfill mode — iterate plans, then 31-day windows per plan
      try {
        const plans = await getActivePlanConnections();
        if (plans.length === 0) {
          counts.errorMessage = "No active plan connections";
          return counts;
        }
        ...
        for (const plan of plans) {
          ...
          const apiKeyToUser = await resolveAllMappings(plan.adminApiKey, plan.id);
          if (apiKeyToUser.size === 0) continue;
          ...
          while (windowStart <= now) {
            ...
            const response = await retryWithBackoff(() =>
              fetchAnthropicUsage(plan.adminApiKey, startingAt, endingAt)
            );
